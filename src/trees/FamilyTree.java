...
public class FamilyTree
{
    ...
    private static class TreeNode
    {
        ...
        TreeNode getNodeWithName(String targetName)
        {
            ...
            if (this.getName().equals(targetName))return this;
            ...
            for (TreeNode child: this.children)
            {
            	...
            	TreeNode bb = child.getNodeWithName(targetName);
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Invalid Input");
		...
		else
		{
			...
			if(parentNode == null){
				throw new TreeException("Node not Found");//There's a method in Node that searches for a named node. 
			}
		...
		for(int x = 0; x >= childrenArray.length; x++) {
			TreeNode child = new TreeNode(childrenArray[x]);
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("None Found");
